...
| **Web app — React SPA frontend (§5.14)** | ✅ **v0.4 (2026-05-27)** | React 18 + Vite + TypeScript console with **eight views**, soft mint/cream light theme, plain-English help cards on every page, and per-field tooltips. Views: **🏡 Home** (landing page with tiles + 4-step walkthrough), **🧠 About the Model** (algorithm card + headline raw features + top-15 processed-feature importances + 8-step training pipeline narrated twice via a plain/technical toggle), **🗄️ Data & Analyses** (new — dataset properties, label construction, leakage audit, and every analysis stage from EDA → fairness with empirical findings; plain/technical toggle), **📊 Dashboard** (KPIs + model card), **👤 Score an Applicant** (accepts **raw values** — FICO score, dollar amounts, industry dropdown — with basic/advanced field toggle; backend runs the preprocessor for the user), **📂 Score a Whole CSV** (3-step uploader with downloadable column template), **🔍 What the Model Looks At** (global importance), **⚖️ Fairness Check** (per-axis gaps + Selbst traps). Run: `cd apps/web && npm install && npm run dev` → <http://localhost:5173>. |
...
| The dataset (it's gitignored) | `data/README.md` — acquisition + dataset properties + every analysis stage with findings |
| A plain-English tour of the dataset and the analyses | 🗄️ **Data & Analyses** view in the web app (`apps/web/src/views/DataAndAnalyses.tsx`) |
