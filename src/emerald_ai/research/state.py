...
class State:
    ...
    def save(self, *, papers_count: int | None = None) -> None:
        _write_json(self.root / "processed.json", self.processed)
        _write_json(self.root / "citations.json", _to_jsonable(self.citations))
        _write_json(self.root / "questions.json", _to_jsonable(self.questions))
        _write_json(
